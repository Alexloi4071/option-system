...
class LongCallCalculator:
    """Long Call損益計算器 (100%書籍實現)
    公式: 損益 = Max(股價-行使價,0) - 期權金
    書籍來源: 《期權制勝》第二課"""
    ...
    def calculate_current_pnl(
        ...
        參數:
            ...
            current_stock_price: 當前股價（非到期股價）
        ...
        """
        try:
            logger.info(f"開始計算 Long Call 當前持倉損益...")
            ...
            logger.info(f"* Long Call 當前持倉損益計算完成")
            ...
            logger.error(f"x Long Call 當前持倉損益計算失敗: {e}")
            raise
