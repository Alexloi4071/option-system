...
class ShortPutCalculator:
    ...
    def calculate_current_pnl(
        ...
        對於 Short Put，損益 = 收取的期權金 - 當前期權價格（平倉成本）
        ...
        """
        try:
            logger.info(f"開始計算 Short Put 當前持倉損益...")
            ...
            logger.info(f"* Short Put 當前持倉損益計算完成")
            ...
            logger.error(f"x Short Put 當前持倉損益計算失敗: {e}")
            raise
