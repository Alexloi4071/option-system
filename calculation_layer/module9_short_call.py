...
class ShortCallCalculator:
    ...
    def calculate(self, strike_price: float, option_premium: float,
        ...
        try:
            ...
            logger.error(f"x Short Call計算失敗: {e}")
            raise
    ...
    def calculate_with_contracts(
        ...
        """
        計算多張合約的 Short Call 損益
        
        參數:
            strike_price: 行使價
            option_premium: 每股期權金（收取）
        ...
        multiplier = 100
        total_premium_received = option_premium * num_contracts * multiplier
        ...
        return {
            **result.to_dict(),
            ...
            'total_return_percentage': round((total_pnl / total_premium_received) * 100, 2) if total_premium_received > 0 else 0
        }
    ...
    def calculate_current_pnl(
        ...
        對於 Short Call，損益 = 收取的期權金 - 當前期權價格（平倉成本）
        
        參數:
            strike_price: 行使價
            option_premium: 賣出時收取的期權金（每股）
            current_stock_price: 當前股價
            current_option_price: 當前期權市場價格（平倉成本）
            num_contracts: 合約數量
            calculation_date: 計算日期
        ...
        """
        try:
            logger.info(f"開始計算 Short Call 當前持倉損益...")
            ...
            # 當前內在價值 (Call: max(S-K, 0))
            intrinsic_value = max(current_stock_price - strike_price, 0)
            
            # 當前時間價值
            time_value = max(current_option_price - intrinsic_value, 0)
            
            # Short 損益 = 收取的期權金 - 當前期權價格
            unrealized_pnl_per_share = option_premium - current_option_price
            
            # 總收入和總損益
            total_premium_received = option_premium * num_contracts * multiplier
            total_buyback_cost = current_option_price * num_contracts * multiplier
            ...
            logger.info(f"* Short Call 當前持倉損益計算完成")
            ...
            return {
                ...
                'num_contracts': num_contracts,
                'total_premium_received': round(total_premium_received, 2),
                'total_buyback_cost': round(total_buyback_cost, 2),
            ...
            logger.error(f"x Short Call 當前持倉損益計算失敗: {e}")
            raise
